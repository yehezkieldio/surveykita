        ...
        <x-card class="!p-4">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <form method="GET" action="{{ route('admin.results.show', $form) }}" class="flex flex-wrap gap-3">
                    <select name="category_id" class="block text-xs">
            ...
            <x-card heading="Rekap Kategori" subheading="Skor rata-rata berdasarkan kategori pertanyaan.">
                <div class="overflow-x-auto mt-2">
                    <table class="w-full text-left text-sm divide-y divide-zinc-200">
                        <thead>
                            <tr class="text-zinc-500 font-medium text-xs border-b border-zinc-200">
                                <th class="px-4 py-3.5 text-left font-medium">Kategori</th>
                                <th class="px-4 py-3.5 text-left font-medium">Jawaban</th>
                                <th class="px-4 py-3.5 text-left font-medium">Rata-rata</th>
                                <th class="px-4 py-3.5 text-left font-medium">Kepuasan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-100">
                            @foreach ($result['average_score_per_category'] as $row)
                                <tr class="hover:bg-zinc-50/50 transition-colors duration-150 text-zinc-950">
                                    <td class="px-4 py-3 font-semibold">{{ $row['category'] }}</td>
                                    <td class="px-4 py-3 text-zinc-500 font-mono text-xs">{{ $row['total_answers'] }}</td>
                                    <td class="px-4 py-3 font-mono font-semibold text-zinc-900">{{ $row['average_score'] }}</td>
                                    <td class="px-4 py-3">
            ...
            <x-card heading="Distribusi Skala Likert" subheading="Frekuensi skor jawaban dari skala 1 sampai 5.">
                <div class="grid grid-cols-5 gap-3 mt-4">
                    @foreach ($result['likert_distribution'] as $score => $count)
                        <div class="rounded-lg bg-zinc-50 p-4 text-center hover:bg-zinc-100/50 transition-colors duration-200">
                            <p class="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium">Skor {{ $score }}</p>
                            <p class="mt-2 text-2xl font-bold tracking-tight text-zinc-900">{{ $count }}</p>
                        </div>
                    @endforeach
                </div>
            </x-card>
        ...
        <x-card heading="Rekap Detail Pertanyaan" subheading="Analisis tingkat kepuasan per butir pertanyaan.">
            <div class="overflow-x-auto mt-2">
                <table class="w-full text-left text-sm divide-y divide-zinc-200">
                    <thead>
                        <tr class="text-zinc-500 font-medium text-xs border-b border-zinc-200">
                            <th class="px-4 py-3.5 text-left font-medium">Pertanyaan</th>
                            <th class="px-4 py-3.5 text-left font-medium">Kategori</th>
                            <th class="px-4 py-3.5 text-left font-medium">Jawaban</th>
                            <th class="px-4 py-3.5 text-left font-medium">Rata-rata</th>
                            <th class="px-4 py-3.5 text-left font-medium">Kepuasan</th>
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @foreach ($result['average_score_per_question'] as $row)
                            <tr class="hover:bg-zinc-50/50 transition-colors duration-150 text-zinc-950">
                                <td class="px-4 py-3.5 font-medium leading-relaxed max-w-sm">{{ $row['question_text'] }}</td>
                                <td class="px-4 py-3.5 text-zinc-500 text-xs">{{ $row['category'] }}</td>
                                <td class="px-4 py-3.5 font-mono text-zinc-500 text-xs">{{ $row['total_answers'] }}</td>
                                <td class="px-4 py-3.5 font-mono font-semibold text-zinc-900">{{ $row['average_score'] }}</td>
