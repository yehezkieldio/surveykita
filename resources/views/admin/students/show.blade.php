                ...
                @else
                    <x-ui.table :headers="['Formulir', 'Periode', 'Tanggal Kirim']">
                        @foreach ($student->responses as $response)
                            <tr>
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-semibold text-zinc-950">{{ $response->evaluationForm->title }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-zinc-600">{{ $response->evaluationForm->evaluationPeriod->name }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-zinc-600">{{ $response->submitted_at->translatedFormat('d M Y, H:i') }}</td>
                            </tr>
                        @endforeach
                    </x-ui.table>
                @endif
            ...
            <div class="rounded-lg border border-zinc-200 bg-white p-6">
                ...
                <div class="mt-4 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <x-ui.badge variant="teal">{{ $student->user?->role }}</x-ui.badge>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-medium text-zinc-500">Terdaftar Pada</span>
                        <span class="text-xs font-semibold text-zinc-950">{{ $student->created_at->translatedFormat('d M Y') }}</span>
                    </div>
